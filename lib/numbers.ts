...
export function convert(n) {
  ...
  if (trailingZeros.length === 0) {
    return `${formatNumber(+integerPart)}.${nonZeroDigits}`;
  } else {
    return `${formatNumber(+integerPart)}.0${getSubscriptForNumber(
      trailingZeros.length
    )}${nonZeroDigits}`;
  }
}
