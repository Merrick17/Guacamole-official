...
export default function Roulette() {
  ...
  const setSelectedBetAmount = useRoulette(
    (state) => state.setSelectedBetAmount
  );
      ...
      tableBet.numbers.map((value, i) => {
        return Object.keys(NAMED_BETS).reduce((prev, key) => {
          ...
          return ids.includes(i)
            ? Math.floor(prev + tableBet.named[betName] / ids.length)
            : prev;
        }, value);
      }),
    [tableBet]
  ...
  const { wager, bet, maxPayoutExceeded, maxPayout } = useMemo(() => {
    ...
    const bet = distributedBet.map(
      (amount) => (amount * distributedBet.length) / (wager || 1)
    );
  ...
  const play = async () => {
    try {
      ...
      if (result.payout > 0) soundWin.start();
    } catch {
      //
    } finally {
      setLoading(false);
  ...
  return (
    <>
        ...
        <div className=" grid gap-5">
          ...
          >
            <div>
              ...
              <div style={{ fontSize: '10px' }}>MAX PAYOUT</div>
            </div>
            <div>
              <div style={{ fontWeight: 'bold' }}>{formatLamports(wager)}</div>
              <div style={{ fontSize: '10px' }}>TOTAL BET</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-evenly' }}>
            {CHIPS.map((value) => (
              ...
              >
                <Chip
                  inactive={value !== selectedBetAmount}
                  value={lamportsToSol(value)}
                >
                  {lamportsToSol(value)}
                </Chip>
              </StylelessButton>
      ...
      <ActionBar>
        ...
        <Button
          disabled={loading || maxPayoutExceeded || !wager}
          onClick={play}
        >
          Spin
        </Button>
