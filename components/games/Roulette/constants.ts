...
export const CHIPS = [0.01, 0.05, 0.1, 0.25, 0.5].map(solToLamports);
...
const squares = Array.from({ length: NUMBERS })
  .fill(0)
  .map((_, i) => i);
...
export const INITIAL_TABLE_BETS = {
  ...
  named: Object.keys(NAMED_BETS).reduce(
    (prev, key) => ({ ...prev, [key]: 0 }),
    {} as Record<NamedBet, number>
  ),
};
