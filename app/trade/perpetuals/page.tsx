...
import { Leverage, Price, dexterity, useManifest, useProduct, useTrader } from '@/context/dexterity';
  ...
  const updateLeverage = useCallback(async() => {
    if (trader && markPrice) {
      ...
      for (const [productName, obj] of dexterity.Manifest.GetProductsOfMPG(
        ...
        const leverage: number = Number((markPrice.find((p) => p.index === productIndex).price / currMarginReq).toFixed(2))
